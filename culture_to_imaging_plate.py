...
# containers
tiprack = containers.load('tiprack-200ul_accurate', 'A1')
...
trash = containers.load('tiprack-200ul_accurate', 'D2') # [!INFO!] This is just a trash can. I suggest calibrating this to the middle of the container. It's only ever going to dispense in "A1"
...
# pipette in stain
volume = 50
for i, (_, destination) in enumerate(zip(cell_culture.wells(), imaging_plate.wells())):
    if i > (num_wells - 1): break
    sterile_pipetting.transfer(volume, stain.wells('A1'), destination, mix_after=True)
